	...
	window[fnName] = function(changeConfig){
		...
		Img.onload = function(){
			...
			for(i = 0; i < containerItem.length; i++){
				...
				var xT ,yT;
				if (_default.type=='top') {
					xT = 0;
					yT = Math.ceil(Math.random()*windowHeight) - windowHeight;
				} else if(_default.type=='bottom'){
					xT = 0;
					yT = Math.ceil(Math.random()*windowHeight*2);
				}else{
					xT = Math.ceil(Math.random()*windowWidth*2) - windowWidth;
					yT = Math.ceil(Math.random()*windowHeight*2) - windowHeight;
				}
